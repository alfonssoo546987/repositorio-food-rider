<%-- 
    ...
    Created on : 27 abr 2025, 0:35:17
    Author     : alfon
--%>
...
<html>
    ...
    <body>
        ...
        <h2><fmt:message key="titulo.administrador"/></h2>
        <core:forEach var="elementoAdministrador" items="${administradoresAt}">
            <core:forEach var="elementoUsuario" items="${usuariosAt}">
                <core:if test="${elementoUsuario.id_usuario == elementoAdministrador.usuario.id_usuario}">
                    ...
                    <p><fmt:message key="campo.dni"/>${elementoAdministrador.numero_documento_administrador}</p>
                    <p><fmt:message key="campo.nss"/>${elementoAdministrador.nss}</numero_documento_administradorp>
                <p><fmt:message key="campo.nombre_propio"/>${elementoAdministrador.nombre}</p>
                <p><fmt:message key="campo.primer_apellido"/>${elementoAdministrador.apellido_principal}</p>
                <p><fmt:message key="campo.segundo_apellido"/>${elementoAdministrador.apellido_secundario}</p>
            </core:if>
        </core:forEach>
    </core:forEach>
